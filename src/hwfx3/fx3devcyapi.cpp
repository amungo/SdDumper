...
fx3_dev_debug_info_t FX3DevCyAPI::getDebugInfoFromBoard(bool ask_speed_only) {
    ...
    if ( ask_speed_only ) {
        ...
        fx3_dev_debug_info_t info;
        UCHAR* buf = (UCHAR*) info.data.data();
        std::fill( &buf[0], &buf[info.dataSize8() - 1], 0 );
...
void FX3DevCyAPI::sendCommand(uint32_t code, void *buffer, int len) {
    ...
    CtrlEndPt->ReqType   = REQ_VENDOR;
    CtrlEndPt->Direction = DIR_FROM_DEVICE;
    CtrlEndPt->ReqCode = code;
    CtrlEndPt->Value = 0;
    CtrlEndPt->Index = 1;
    LONG llen = len;
    int success = CtrlEndPt->XferData( &buf[0], llen );
    if ( !success ) {
        fprintf( stderr, "FX3DevCyAPI::sendCommand( 0x%02X, len = %d ) __error__  CtrlEndPt->XferData is FALSE\n", code, len );
    }
}
